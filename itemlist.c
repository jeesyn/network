...
{
    struct list_head *llist, *tmplist, *tplist;
    struct item* finditem;
    unsigned long index = item->mpacket.index;
    int atlast = 1;
    ITEM_LOCK(itemlist);
    if (list_empty(&itemlist->list))/*itemlist is empty*/
    {
         list_add_tail(&item->list, &itemlist->list);
         itemlist->item_count++;
         ITEM_UNLOCK(itemlist);
    ...
    if (itemlist->max_items > 0 && itemlist->max_items <= itemlist->item_count) {
     ...
     list_for_each_safe(llist, tmplist, &itemlist->list) {
        finditem = list_entry(llist, struct item, list);
        if (index < finditem->mpacket.index) {
            tplist = &finditem->list;
            __list_add(&item->list, tplist->prev, &finditem->list);
            atlast = 0;
            break;
        }
    }
